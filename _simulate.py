"""
SimulationRunner - ENHANCED VERSION with new callbacks integrated.

This is a modified version of simulate.py that includes the NEW custom callbacks:
1. HDF5ThermalSaver - Efficient compressed thermal field storage
2. HDF5ActivationSaver - Efficient compressed activation volume storage
3. PerspectiveCameraCallback - Camera that follows the nozzle with overlay

DIFFERENCE FROM simulate.py:
- Line 423: get_default_callbacks() is replaced with get_enhanced_callbacks()
- New function get_enhanced_callbacks() combines original + new callbacks
- Everything else remains IDENTICAL to the original simulate.py

USAGE:
    python _simulate.py --part-x 5.0 --part-y 5.0 --part-z 2.0 --laser-power 800
"""
...
# ==============================================================================
# NEW FUNCTION: Get enhanced callbacks (original + new custom callbacks)
# ==============================================================================
def get_enhanced_callbacks():
    """
    Get callback collection that includes BOTH original and NEW custom callbacks.

    Returns:
        List of callback instances combining:
        - Original callbacks from get_default_callbacks()
        - NEW HDF5ThermalSaver
        - NEW HDF5ActivationSaver
    ...
    # Add NEW HDF5 thermal field saver
    callbacks.append(
        HDF5ThermalSaver(
            filename="thermal_fields.h5",
            interval=5,  # Save every 5 steps (adjust as needed)
            compression='gzip',
            compression_opts=4  # Moderate compression for speed
        )
    )

    # Add NEW HDF5 activation volume saver
    callbacks.append(
        HDF5ActivationSaver(
    ...
    # Add NEW perspective camera callback with overlay
    callbacks.append(
        PerspectiveCameraCallback(
            ...
            save_images=True,
            save_dir="cam",                        # Save to cam/ subdirectory
            interval=5,                            # Save every 5 steps
            resolution_wh=(800, 600),              # Image resolution
            enable_overlay=True,                   # Enable nozzle + powder overlay
            overlay_config={
...
if __name__ == "__main__":
    ...
    # NOTE: We do NOT import get_default_callbacks here, we use our new function

    parser = argparse.ArgumentParser(
        description='Run DED simulation with ENHANCED callbacks (original + HDF5 + camera)'
    )
    ...
    # =========================================================================
    # MAIN DIFFERENCE: Use get_enhanced_callbacks() instead of get_default_callbacks()
    # =========================================================================
    print("\n" + "="*80)
    print("RUNNING ENHANCED SIMULATION (_simulate.py)")
    print("="*80)
    print("\nThis version includes ALL callbacks:")
    print("  ✓ Original callbacks (same as simulate.py)")
    print("  ✓ NEW HDF5ThermalSaver (compressed thermal fields)")
    print("  ✓ NEW HDF5ActivationSaver (compressed activation volumes)")
    print("  ✓ NEW PerspectiveCameraCallback (following camera with overlay)")
    ...
    print("  - cam/ directory (perspective camera images)")
    print("="*80 + "\n")

    callbacks = get_enhanced_callbacks()  # ← ONLY CHANGE from simulate.py
    ...
    print("\n" + "="*80)
    print("CHECK FOR NEW OUTPUTS:")
    print("="*80)
    print("  [ ] thermal_fields.h5 - Compressed thermal field history")
    print("  [ ] activation_volumes.h5 - Compressed activation volume history")
    print("  [ ] cam/ - Perspective camera images with nozzle overlay")
    print("  [ ] All original outputs (CSV, NPY, PNG) should also be present")
    print("="*80 + "\n")
