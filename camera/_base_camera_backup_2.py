...
class BaseCamera:
    ...
    def render_first_hit(self,
        ...
        if self.fill_gaps:
            ...
            img = self._fill_gaps_simple(img, depth_map, ambient)
            print("Gap filling applied.")

        return img, extent

    def _fill_gaps_simple(self, img: np.ndarray, depth_map: np.ndarray, ambient: float) -> np.ndarray:
        """
        8-connectivity gap filling with diagnostics.
        """
        W, H = img.shape
        has_hit = (img != ambient)
        img_filled = img.copy()
        ...
        for ix in range(1, W - 1):
            for iy in range(1, H - 1):
                ...
                # Check 8-neighbors (INCLUDES DIAGONALS!)
                neighbors = [
                ...
                for nx, ny in neighbors:
                    if has_hit[nx, ny]:
                        neighbor_temps.append(img[nx, ny])
                        neighbor_depths.append(depth_map[nx, ny])

                # Need at least 1 neighbor (CHANGED FROM 2!)
                if len(neighbor_depths) < 1:
                    rejected_no_neighbors += 1
                    continue

                neighbor_depths = np.array(neighbor_depths)
                neighbor_temps = np.array(neighbor_temps)

                # If only 1 neighbor, just use it (no depth check needed)
                if len(neighbor_depths) == 1:
                    img_filled[ix, iy] = neighbor_temps[0]
                    gaps_filled += 1
                else:
                    # Multiple neighbors: check depth consistency
                    depth_range = np.max(neighbor_depths) - np.min(neighbor_depths)

                    if depth_range <= self.depth_tolerance:
                        img_filled[ix, iy] = np.mean(neighbor_temps)
                        gaps_filled += 1
                    else:
                        rejected_depth_mismatch += 1
                        depth_mismatches.append(depth_range)

        # Print diagnostics
        print(f"\n{'=' * 70}")
        print(f"GAP FILLING DIAGNOSTICS")
        print(f"{'=' * 70}")
        print(f"Total interior gaps: {total_gaps}")
        print(f"Gaps filled: {gaps_filled} ({gaps_filled / total_gaps * 100:.1f}%)")
        print(f"Rejected (< 1 neighbor): {rejected_no_neighbors}")
        print(f"Rejected (depth mismatch): {rejected_depth_mismatch}")
        print(f"Depth tolerance: {self.depth_tolerance * 1e6:.1f} µm")

        if depth_mismatches:
            depth_mismatches = np.array(depth_mismatches) * 1e6
            print(f"\nDepth mismatches (rejected gaps):")
            print(f"  Min: {np.min(depth_mismatches):.1f} µm")
            print(f"  Median: {np.median(depth_mismatches):.1f} µm")
            print(f"  Max: {np.max(depth_mismatches):.1f} µm")

        print(f"{'=' * 70}\n")

        return img_filled

    def _fill_gaps_simple_debug(self, img: np.ndarray, depth_map: np.ndarray, ambient: float) -> np.ndarray:
        """
        Instrumented version - prints diagnostics.
        """
        W, H = img.shape
        has_hit = (img != ambient)
        img_filled = img.copy()
        ...
        # Statistics
        total_gaps = 0
        gaps_with_neighbors = 0
        gaps_filled = 0
        rejected_depth_mismatch = 0
        rejected_no_neighbors = 0

        depth_mismatches = []  # Track how bad the mismatches are
        ...
        for ix in range(1, W - 1):
            for iy in range(1, H - 1):
                ...
                if len(neighbor_depths) < 2:
                    rejected_no_neighbors += 1
                    continue

                gaps_with_neighbors += 1
                neighbor_depths = np.array(neighbor_depths)

                # Check depth consistency (same surface)
                depth_range = np.max(neighbor_depths) - np.min(neighbor_depths)
        ...
        # Print diagnostics
        print("\n" + "=" * 70)
        print("GAP FILLING DIAGNOSTICS")
        print("=" * 70)
        print(f"Total interior gaps: {total_gaps}")
        print(f"Gaps with 2+ neighbors: {gaps_with_neighbors}")
        print(f"Gaps filled: {gaps_filled} ({gaps_filled / total_gaps * 100:.1f}%)")
        print(f"Rejected (< 2 neighbors): {rejected_no_neighbors}")
        print(f"Rejected (depth mismatch): {rejected_depth_mismatch}")
        print(f"\nDepth tolerance: {self.depth_tolerance * 1e6:.1f} µm")

        if depth_mismatches:
            depth_mismatches = np.array(depth_mismatches)
            print(f"\nDepth mismatches (rejected gaps):")
            print(f"  Min: {np.min(depth_mismatches) * 1e6:.1f} µm")
            print(f"  Mean: {np.mean(depth_mismatches) * 1e6:.1f} µm")
            print(f"  Max: {np.max(depth_mismatches) * 1e6:.1f} µm")
            print(f"  Median: {np.median(depth_mismatches) * 1e6:.1f} µm")

            # Suggest better tolerance
            suggested = np.median(depth_mismatches) * 1.5
            print(f"\nSuggested depth_tolerance: {suggested * 1e6:.1f} µm ({suggested:.6f} m)")

        print("=" * 70 + "\n")

        return img_filled

    def _fill_gaps_simple_old(self, img: np.ndarray, depth_map: np.ndarray, ambient: float) -> np.ndarray:
        """
        Simple depth-aware gap filling using 8-connectivity.

        Only fills gaps where 8-neighbors (including diagonals) have hits at similar depths (same surface).
        """
        W, H = img.shape
        has_hit = (img != ambient)
        img_filled = img.copy()
        ...
        for ix in range(1, W - 1):
            for iy in range(1, H - 1):
                ...
                # Check 8-neighbors (includes diagonals for single-pixel gaps)
                neighbors = [
                    (ix - 1, iy - 1), (ix - 1, iy), (ix - 1, iy + 1),
                    (ix, iy - 1), (ix, iy + 1),
                    (ix + 1, iy - 1), (ix + 1, iy), (ix + 1, iy + 1)
                ...
                # Need at least 1 neighbor (changed from 2 for single-pixel gaps)
                if len(neighbor_depths) >= 1:
                    neighbor_depths = np.array(neighbor_depths)
                    neighbor_temps = np.array(neighbor_temps)
                    ...
                    # Check depth consistency (same surface)
                    if len(neighbor_depths) == 1:
                        # Only 1 neighbor: just use it (no depth check possible)
                        img_filled[ix, iy] = neighbor_temps[0]
                    else:
                        # Multiple neighbors: check depth consistency
                        if np.max(neighbor_depths) - np.min(neighbor_depths) <= self.depth_tolerance:
                            # Fill with average
                            img_filled[ix, iy] = np.mean(neighbor_temps)
