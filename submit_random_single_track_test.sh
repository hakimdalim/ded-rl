#!/bin/bash
#SBATCH --job-name=ded_random_single_track_test
#SBATCH --nodes=1
#SBATCH --ntasks=16
...
# Test submission script for random material single-track experiments
# Reduced parameter set: 4×2×2×2 = 32 parameter combinations
# With 3 material variants each = 96 total experiments
# 16 parallel experiments × 1 CPU each
# Uses SLURM steps instead of GNU Parallel

echo "=========================================="
echo "DED Random Material Single-Track TEST Run"
echo "=========================================="
...
EXPERIMENTS_PER_BATCH=96  # 32 parameter combinations × 3 material variants
TOTAL_EXPERIMENTS=96

# Shared directories
POWDER_STREAM_DIR="/scratch/schuermm-shared/sim_powder_stream_arrays"
MATERIALS_DIR="/scratch/schuermm-shared/random_materials_TEST"  # Directory with generated material JSON files
OUTPUT_BASE_DIR="/scratch/schuermm-shared/ded_random_single_track_TEST"
...
# Test design: 4 powers × 2 diameters × 2 feeds × 2 speeds × 3 variants = 96 experiments
PARAM_FILE="$BATCH_OUTPUT_DIR/parameters.txt"
> "$PARAM_FILE"  # Clear file

# Reduced parameter ranges for testing
POWERS=(600 900 1200 1500)           # 4 values
DIAMETERS=(0.5 1.8)                  # 2 values
FEEDS=(2.0 4.0)                      # 2 values
SPEEDS=(2 20)                        # 2 values
VARIANTS=(0 1 2)                     # 3 material variants per parameter combination
    ...
    for diameter in "${DIAMETERS[@]}"; do
        for feed in "${FEEDS[@]}"; do
            for speed in "${SPEEDS[@]}"; do
                # For each parameter combination, create 3 experiments with different material variants
                for variant in "${VARIANTS[@]}"; do
                    # Check if this experiment belongs to current batch
                    if [ $experiment_idx -ge $START_IDX ] && [ $experiment_idx -le $END_IDX ]; then
                        # Material name format matches what generate_random_materials.py creates
                        MATERIAL="random_P${power}W_D${diameter}mm_F${feed}gmin_V${speed}mms_v${variant}"
    ...
    srun --ntasks=1 \
         ...
         bash -c "
           ...
           python run_single_track_experiment.py \
             ...
             --material-file-mode single-file \
...
OUTPUT_DIRS=$(find "$BATCH_OUTPUT_DIR/experiments" -name "random_P*" -type d 2>/dev/null | wc -l)
