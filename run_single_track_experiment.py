...
def parse_args():
    ...
    parser.add_argument('--nozzle-height-mm', type=float, default=15.0,
                       help='Nozzle height above substrate in mm')
...
def main():
    ...
    params.request_change(
        ...
        nozzle_height=args.nozzle_height_mm / 1000.0,  # mm to m
    )

    # Initialize powder stream from npz file
    ...
    voxel_powder_stream = VoxelPowderStream(
        ...
        outlet_offset=1.1635e-3,  # Default from powder_stream_new.py
        nozzle_height=args.nozzle_height_mm / 1000.0,  # mm to m
        visualize=False
    )
