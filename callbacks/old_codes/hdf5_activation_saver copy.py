"""
HDF5-based activation volume saver for efficient storage of 3D activation states.

HDF5 provides:
- Compression (saves disk space - bool arrays compress extremely well!)
...
class HDF5ActivationSaver(IntervalCallback):
    """
    Saves complete 3D activation volume to HDF5 format after each step.

    The activation volume is a boolean array showing which voxels have been
    activated (melted/solidified) during the build process.

    HDF5 format advantages:
    - Extreme compression: ~50-1000x smaller files than .npy for bool arrays
      (bool arrays are very sparse, so they compress extremely well)
    ...
    - Single file: All timesteps in one file

    Usage:
        # Save to single file with all timesteps
        HDF5ActivationSaver(interval=10)

        # Adjust compression (default is already excellent for bool data)
        HDF5ActivationSaver(compression='gzip', compression_opts=9)

    Output:
        activation_volumes.h5
            /step_0001/
            │   ├── activation  [nx, ny, nz] bool array
...
def list_steps_in_file(filepath: str) -> list:
    """
    List all available steps in HDF5 file.
...
if __name__ == "__main__":
    print("""
HDF5ActivationSaver - Efficient activation volume storage
...
Advantages over NPY format:
    - 50-1000x smaller files (bool arrays compress extremely well!)
    - Can store metadata with data
    - Faster I/O for large arrays
    - Can read partial data without loading entire file
    - Track activation evolution over time
""")
